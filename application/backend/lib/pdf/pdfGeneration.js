...
async function generatePdfAndSend(res, next, html, options) {
    try {
        ...
        next(new Error(`PDF Erzeugung nicht erfolgreich wegen (${e?.message})`));
    }
}
export async function kassenbericht(res, next, datum) {
    const now = new DatumUhrzeit();
    generatePdfAndSend(res, next, renderPug("kassenbericht", { datum, now }));
}
export async function vertrag(res, next, konzertUrl, language) {
    ...
    generatePdfAndSend(res, next, renderPug(`vertrag-${sprache}`, {
...
export async function vermietungAngebot(res, next, vermietungUrl, art) {
...
export async function kassenzettel(res, next, konzertUrl) {
...
export async function riderPdf(res, next, url) {
...
export async function kassenzettelToBuchhaltung(konzert) {
...
function renderVermietung(vermietung, now) {
...
async function vermietungToPdf(vermietung) {
    const now = new DatumUhrzeit();
...
export async function vermietungVertragToBuchhaltung(vermietung) {
    if (!conf.kassenzettelEmail) {
        return;
    }
...
export async function saveVermietungToShare(vermietung) {
    if (!conf.pdfuploadpath) {
        return;
    }
    const { content, filename } = await vermietungToPdf(vermietung);
    const directory = Path.join(conf.pdfuploadpath, vermietung.startDatumUhrzeit.jahr.toString(10), `${vermietung.startDatumUhrzeit.monatTag} ${vermietung.kopf.titel}`);
    try {
